...
class WebSocket(object):
    ...
    def once(self):
        ...
        while self.sock.bytesAvailable() > 0:
            logger.debug("Have {} bytes ready to read".format(self.sock.bytesAvailable()))
            try:
                b = self.sock.read(self.reading_buffer_size)
                logger.debug("WebSocket read: {} bytes \n{}".format(b.length(), repr(b)))
                s = b.__str__()
            except socket.error:
                logger.exception("Failed to receive data")
                return False
            else:
                if not self.process(s):
